...
Recent updates:
    -Added module 4 (bond discount/premium amortization calculator)
    -Updated module 6 (A/R turnover calculator)
"""
...
(4) Bond discount/premium amortization calculator
(5) Present/future value calculator
...
if "1" in calcMode:
    ...
    """BOND DISCOUNT/PREMIUM AMORTIZATION CALCULATOR"""
    face = int(input("Enter the face amount of the bond: "))
    stated = float(input("Enter the stated interest rate: "))
    stated /= 100
    mkt = float(input("Enter the market interest rate: "))
    mkt /= 100
    yrs = int(input("Enter the number of years of the bond: "))
    pds = int(input("Enter the number payments per year: "))
    
    total_pymts = yrs * pds
    int_pd = mkt / pds
    pymt_pd = (stated / pds) * face
    
    # Calculate PV factors
    annuity_factor = float((1 - (1 / ((1 + int_pd) ** total_pymts))) / int_pd)
    pv_factor = 1 / ((1 + int_pd) ** total_pymts)
    
    # Calculate issue price of bond
    pv_interest = annuity_factor * pymt_pd
    pv_face = pv_factor * face
    issue_price = pv_interest + pv_face
    
    if mkt == stated:
        # Par    
        price = float(round(issue_price / face * 100, 1))
        carry_value = issue_price
        
        print("=" * 54)
        print("%37s" % "**Issuance Details**\n")
        print("%10s%5s" % ("Issue Price: ", price))
        print("-" * 54)
        print("%40s" % "**Amortization Schedule**\n")
        print("%4s%15s%18s%15s" % ("Period", "Cash Payment", "Interest Expense", "Carry Value"))
        print("-" * 54)
        print("%4d%50s" % (0, bn.format_currency(de.Decimal(carry_value),"USD")))
        
        period = 1
        while period < (total_pymts + 1):
            print("%4s%15s%18s%17s" % (period, bn.format_currency(de.Decimal(pymt_pd),"USD"), bn.format_currency(de.Decimal(pymt_pd),"USD"), bn.format_currency(de.Decimal(carry_value),"USD")))
            period += 1
            
        print("-" * 54)
        print("%10s%5s" % ("Total Interest Expense: ", bn.format_currency(de.Decimal(pymt_pd * total_pymts),"USD")))
        print("=" * 54)
    
    elif mkt < stated:
        # Premium
        price = float(round(issue_price / face * 100, 1))
        prem = issue_price - face
        carry_value = issue_price
        
        print("=" * 74)
        print("%47s" % "**Issuance Details**\n")
        print("%10s%5s" % ("Issue Price: ", price))
        print("%10s%5s" % ("Bond Premium: ", bn.format_currency(de.Decimal(prem),"USD")))
        print("-" * 74)
        print("%50s" % "**Amortization Schedule**\n")
        print("%4s%15s%18s%22s%13s" % ("Period", "Cash Payment", "Interest Expense", "Principal Repayment", "Carry Value"))
        print("-" * 74)
        print("%4d%69s" % (0, bn.format_currency(de.Decimal(carry_value),"USD")))
        
        period = 1
        total_int = 0
        while period < (total_pymts + 1):
            int_exp = int_pd * carry_value
            amort =  pymt_pd - int_exp
            carry_value -= amort
            
            print("%4s%15s%18s%18s%18s" % (period, bn.format_currency(de.Decimal(pymt_pd),"USD"), bn.format_currency(de.Decimal(int_exp),"USD"), bn.format_currency(de.Decimal(amort),"USD"), bn.format_currency(de.Decimal(carry_value),"USD")))
            
            total_int += int_exp
            period += 1
            
        print("-" * 74)
        print("%10s%5s" % ("Total Interest Expense: ", bn.format_currency(de.Decimal(total_int),"USD")))
        print("=" * 74)
        
    else:
        # Discount
        price = float(round(issue_price / face * 100, 1))
        disc = face - issue_price
        carry_value = issue_price
        
        print("=" * 74)
        print("%47s" % "**Issuance Details**\n")
        print("%10s%5s" % ("Issue Price: ", price))
        print("%10s%5s" % ("Bond Discount: ", bn.format_currency(de.Decimal(disc),"USD")))
        print("-" * 74)
        print("%50s" % "**Amortization Schedule**\n")
        print("%4s%15s%18s%22s%13s" % ("Period", "Cash Payment", "Interest Expense", "Principal Repayment", "Carry Value"))
        print("-" * 74)
    ...
    print("""\n\nEnsure that the list is formatted as a CSV file in the same folder as this Python file. It must be in the format (tabular spreadsheet or comma-separated text file)):
    ...
    # Calculate Days Receivables Outstanding
    dro = 365 / turnover_ratio
    ...
    print("%11s%17s" % ("A/R Turnover Ratio:", turnover_ratio))
    ...
    print("Invalid entry. Please choose 1, 2, 3, 4, 5, or 6.\nProgram restarting.")